    ...
    for probe_i in 0..256 {
        ...
        loop {
            ...
            if current_node.children[0] == u32::MAX {
                ...
                // Leaf nodes have no children, so pop from stack.
                if node_stack_i != 0 {
                    node_stack_i -= 1;
                ...
                // Branch node
                let dist = (probe_global_pos - Vec3::from_array(current_node.sphere_centre)).length();
                if dist < current_node.sphere_radius {
                    let left_child_i = current_node.children[0];
                    let right_child_i = current_node.children[1];
                    ...
                    match [left_child_in_range, right_child_in_range] {
                        ...
                        [false, false] => {
                            // Branch node is in range, but neither of the children are.
                            // Cast rays towards the branch AABB, and pop from the stack.
                            colour += trace_rays_at_aabb(
                                ray_query,
                                block_item_atlas,
                                block_item_luma_atlas,
...
fn trace_rays_at_aabb(
    ...
    if closest_in_aabb.length() < f32::EPSILON {
        ...
        for ray_i in 0..NUM_RAYS {
            // Ray info
            let ray_start = probe_global_pos;
            let seed = ray_i as u32 * 3 + 0xDEADBEEF;
    ...
    for ray_i in 0..NUM_RAYS {
        ...
        let ray_dir = update_face_matrix * raw_ray_dir;
        ...
        if did_ray_hit {
            ...
            let hit_closest_in_aabb = hit_pos
                .min(global_corner_2)
                .max(global_corner_1);
            if hit_closest_in_aabb.length_squared() <= f32::powi(0.01, 2) {
                result_acc += ray_hit_colour;
            }
        }
    }
    let mut colour = result_acc * area_percent / NUM_RAYS as f32;
    colour.w = 1.0;
    colour
}
