...
type RenderPixelGroupRgba = Simd<Rgba8Ne, { RENDER_PIXEL_GROUP_DIM * RENDER_PIXEL_GROUP_DIM }>;
...
type RenderPixelGroupDepth = [f32; RENDER_MICRO_TILE_DIM * RENDER_MICRO_TILE_DIM];
...
impl Unorm8x16 {
    pub const ONES: Self = Self(u8x16::splat(255));
}
...
impl core::ops::Mul for Unorm8x16 {
    ...
    fn mul(self, rhs: Self) -> Self {
        let mut out = self.0.cast::<u16>() * rhs.0.cast::<u16>();
        out += u16x16::splat(128);
        out += out >> u16x16::splat(8);
        Self((out >> u16x16::splat(8)).cast::<u8>())
    }
}
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_endian = "little")] {
            let [reds, greens, blues, alphas]: [u8x16; 4] = [
                (rgbas & u32x16::splat(0xFF)).cast::<u8>(),
                ((rgbas >> u32x16::splat(8)) & u32x16::splat(0xFF)).cast::<u8>(),
                ((rgbas >> u32x16::splat(16)) & u32x16::splat(0xFF)).cast::<u8>(),
                ((rgbas >> u32x16::splat(24)) & u32x16::splat(0xFF)).cast::<u8>(),
            ];
...
fn rgba_4xunorm8x16_to_u32x16(rgbas: [Unorm8x16; 4]) -> u32x16 {
    let [reds_u32, greens_u32, blues_u32, alphas_u32] = rgbas.map(|unorms| unorms.0.cast::<u32>());
    cfg_if::cfg_if! {
        if #[cfg(target_endian = "little")] {
            let out: RenderPixelGroupRgba = reds_u32
                | (greens_u32 << u32x16::splat(8))
                | (blues_u32 << u32x16::splat(16))
                | (alphas_u32 << u32x16::splat(24));
        } else if #[cfg(target_endian = "big")] {
            let out: RenderPixelGroupRgba = alphas_u32
                | (blues_u32 << u32x16::splat(8))
                | (greens_u32 << u32x16::splat(16))
                | (reds_u32 << u32x16::splat(24));
        }
    }
    out
}
...
#[inline(always)]
fn f32x16_to_unorm8x16(ns: f32x16) -> Unorm8x16 {
    Unorm8x16(
        (ns * f32x16::splat(255.0))
            .round()
            .simd_max(f32x16::splat(0.0))
            .simd_min(f32x16::splat(255.0))
            .cast::<u8>(),
    )
}
...
impl From<egui::Color32> for Rgba {
    ...
    fn from(value: egui::Color32) -> Self {
        Self::from_array(egui::Rgba::from(value).to_rgba_unmultiplied())
    }
}
...
impl TiledTextureRgba {
    ...
    pub fn sample_nearest_simd16(&self, us: f32x16, vs: f32x16) -> [Unorm8x16; 4] {
        // Convert UV coords to texel coords.
        let xs = (f32x16::splat(self.width.get() as f32) * us - f32x16::splat(0.5))
            .round()
            .simd_max(f32x16::splat(0.0))
            .simd_min(f32x16::splat((self.width.get() - 1) as f32))
            .cast::<u32>();
        let ys = (f32x16::splat(self.height.get() as f32) * vs - f32x16::splat(0.5))
            .round()
            .simd_max(f32x16::splat(0.0))
            .simd_min(f32x16::splat((self.height.get() - 1) as f32))
            .cast::<u32>();
        let tile_xs = xs / u32x16::splat(TEXTURE_TILE_PIXEL_DIM as u32);
        let tile_ys = ys / u32x16::splat(TEXTURE_TILE_PIXEL_DIM as u32);
        let xs_in_tile = xs % u32x16::splat(TEXTURE_TILE_PIXEL_DIM as u32);
        let ys_in_tile = ys % u32x16::splat(TEXTURE_TILE_PIXEL_DIM as u32);
        let power_of_two_tiles_per_row = self.power_of_two_tiles_per_row.get() as u32;
        let micro_tile_xs = xs_in_tile / u32x16::splat(TEXTURE_MICRO_TILE_DIM as u32);
        let micro_tile_ys = ys_in_tile / u32x16::splat(TEXTURE_MICRO_TILE_DIM as u32);
        let xs_in_micro_tile = xs_in_tile % u32x16::splat(TEXTURE_MICRO_TILE_DIM as u32);
        let ys_in_micro_tile = ys_in_tile % u32x16::splat(TEXTURE_MICRO_TILE_DIM as u32);
        // Traverse nested arrays to get to RGBA values stored in micro-tiles.
        let tile_ptrs = Simd::splat(self.tiles.as_ptr()).wrapping_add(
            ((tile_ys << u32x16::splat(power_of_two_tiles_per_row)) | tile_xs).cast::<usize>(),
        );
        let micro_tile_ptrs = tile_ptrs
            .cast::<[TextureMicroTileRgba; TEXTURE_TILE_DIM]>()
            .wrapping_add(micro_tile_ys.cast::<usize>())
            .cast::<TextureMicroTileRgba>()
            .wrapping_add(micro_tile_xs.cast::<usize>());
        let rgba_ptrs = micro_tile_ptrs
            .cast::<[Rgba8Ne; TEXTURE_MICRO_TILE_DIM]>()
            .wrapping_add(ys_in_micro_tile.cast::<usize>())
            .cast::<Rgba8Ne>()
            .wrapping_add(xs_in_micro_tile.cast::<usize>());
        // Load RGBAs.
        let rgbas: u32x16 = unsafe { Simd::gather_ptr(rgba_ptrs) };
    ...
    pub fn sample_nearest_simd16_masked(
        ...
        // Load RGBAs.
        let rgbas: u32x16 =
            unsafe { Simd::gather_select_ptr(rgba_ptrs, enable.cast::<isize>(), u32x16::splat(0)) };
        rgba_u32x16_to_unorm8x16(rgbas)
    }
}
        ...
        if actual_tile_width < RENDER_TILE_PIXEL_DIM || actual_tile_height < RENDER_TILE_PIXEL_DIM {
            ...
            for y_in_tile in 0..actual_tile_height {
                ...
                for (x_in_tile, pixel) in pixels.iter_mut().enumerate() {
                    ...
                    let rgba8 = pixel_group.as_array()
                    ...
                    for (pixel_group_y, pixel_group_row) in micro_tile.iter().enumerate() {
                            ...
                            pixel_group_row.iter().copied().enumerate()
                        {
                            ...
                            pixel_group
                                .extract::<0, 4>()
                                .to_ne_bytes()
                                .copy_to_slice(unsafe {
                                    core::slice::from_raw_parts_mut(
                                        out_framebuffer.data.add(
                                            tile_start_byte
                                                + (y_in_tile * row_byte_len)
                                                + x_byte_offset,
                                        ),
                                        PIXEL_BYTE_SIZE * 4,
                                    )
