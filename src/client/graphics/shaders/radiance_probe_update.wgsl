...
@group(2) @binding(8)
var<storage, read_write> output_lightmap: array<array<atomic<u32>, 128>>;
@group(2) @binding(9)
var<storage, read> cascade_1_lightmap: array<array<array<u32, 2>, 64>>;
@group(2) @binding(10)
...
fn get_probe_local_position(probe_i: u32) -> vec3<f32> {
    ...
    let probe_x_pos = fma(f32(probe_x), 1.0 / 16.0, 1.0 / 32.0);
    let probe_z_pos = fma(f32(probe_z), 1.0 / 16.0, 1.0 / 32.0);
...
struct Cascade1NearestProbeInfo {
    probes: array<Cascade1NearestProbe, 4>,
    interp_divisor: f32,
}

struct Cascade1NearestProbe {
    index: u32,
    interp_factor: f32,
}

fn get_nearest_cascade_1_probes(cascade_0_probe_pos: vec2<f32>) -> Cascade1NearestProbeInfo {
    let pos_01 = vec2(cascade_0_probe_pos.x + 0.5, 0.5 - cascade_0_probe_pos.y);
    let pos_top_left = pos_01 - (1.0 / 32.0);
    let probe_xz_f32s = pos_top_left * 8.0 / 0.875;
    let upper_left_probe_01 = floor(probe_xz_f32s) * 0.875 / 8.0;
    let upper_right_probe_01 = vec2(ceil(probe_xz_f32s.x), floor(probe_xz_f32s.y)) * 0.875 / 8.0;
    let lower_left_probe_01 = vec2(floor(probe_xz_f32s.x), ceil(probe_xz_f32s.y)) * 0.875 / 8.0;
    let lower_right_probe_01 = ceil(probe_xz_f32s) * 0.875 / 8.0;
    let upper_left_probe_diff = abs(pos_01 - upper_left_probe_01);
    let lower_right_probe_diff = abs(pos_01 - lower_right_probe_01);
    ...
    let floor_xz = floor(probe_xz_f32s);
    let ceil_xz = ceil(probe_xz_f32s);
    let upper_left_probe_xz = min(vec2<u32>(floor_xz), vec2(7u));
    let upper_right_probe_xz = min(vec2<u32>(vec2(ceil_xz.x, floor_xz.y)), vec2(7u));
    let lower_left_probe_xz = min(vec2<u32>(vec2(floor_xz.x, ceil_xz.y)), vec2(7u));
    let lower_right_probe_xz = min(vec2<u32>(ceil_xz), vec2(7u));
    let upper_left_probe_i = upper_left_probe_xz.y * 8 + min(upper_left_probe_xz.x, 7u);
    let upper_right_probe_i = upper_right_probe_xz.y * 8 + min(upper_right_probe_xz.x, 7u);
    let lower_left_probe_i = lower_left_probe_xz.y * 8 + min(lower_left_probe_xz.x, 7u);
    let lower_right_probe_i = lower_right_probe_xz.y * 8 + min(lower_right_probe_xz.x, 7u);
    let probes = array(
        Cascade1NearestProbe(upper_left_probe_i, upper_left_probe_interp_factor),
        Cascade1NearestProbe(upper_right_probe_i, upper_right_probe_interp_factor),
        Cascade1NearestProbe(lower_left_probe_i, lower_left_probe_interp_factor),
        Cascade1NearestProbe(lower_right_probe_i, lower_right_probe_interp_factor),
    );
    let probes_diff = abs(lower_right_probe_01 - upper_left_probe_01);
    return Cascade1NearestProbeInfo(probes, probes_diff.x * probes_diff.y);
}
...
const cascade_ray_length: f32 = 1.0 / 16.0;

@compute
@workgroup_size(1, 256)
fn update_cascade(
    ...
    for (var ray_i: u32 = 0; ray_i < 4; ray_i++) {
        // Ray info
        ...
        var cascade_1_probes = get_nearest_cascade_1_probes(probe_face_local_pos.xz);
        var average_cascade_1_probe_colour = vec3<f32>(0.0);
        var backup_colour: vec3<f32>;
        for (var cascade_1_probe_i: u32 = 0; cascade_1_probe_i < 4; cascade_1_probe_i++) {
            let probe_info = cascade_1_probes.probes[cascade_1_probe_i];
            let ray_pair = cascade_1_lightmap[update_face_i][probe_info.index][ray_i / 2];
            // let ray_pair = 0xFF00FF00u;
            // let ray_pair = cascade_1_lightmap[update_face_i][0][ray_i / 2];
            ...
            let hsv448 = (ray_pair >> shift_amount) & 0xFFFFu;
            let rgb = hsv448_to_rgb(hsv448);
            average_cascade_1_probe_colour += rgb * probe_info.interp_factor;
            backup_colour = rgb;
            // average_cascade_1_probe_colour += hsv448_to_rgb(0xFF00u) * probe_info.interp_factor;
            // backup_colour = hsv448_to_rgb(0xFF00u);
        }
        if cascade_1_probes.interp_divisor < 0.0001 {
            average_cascade_1_probe_colour = backup_colour;
        } else {
            average_cascade_1_probe_colour /= cascade_1_probes.interp_divisor;
        }
        // average_cascade_1_probe_colour /= 4.0;
        // FIXME:
        // - Cascade 1 ray_start is wrong, needs offsetting
        // - Try swapping out average probe colour for RG colour based on pos_01 XZ
        // - Try using cascade 1 for rendering, average over all four probe rays
        let cascade_1_colour = average_cascade_1_probe_colour * (1.0 - ray_hit_colour.a);
        ray_results[ray_i] = ray_hit_colour + vec4(cascade_1_colour, 0.0);
    }
    ...
    let probe_pair = &output_lightmap[update_face_i][probe_i / 2];
    atomicAnd(probe_pair, ~mask);
    var result_acc: vec4<f32> = vec4(0.0);
    for (var result_i: u32 = 0; result_i < 4; result_i++) {
        result_acc += ray_results[result_i];
    }
    let average = result_acc / 4.0;
    atomicOr(probe_pair, rgb_to_hsv448(average.rgb) << shift_amount);
}
